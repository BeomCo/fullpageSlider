$(document).ready(function(){
    ...
    $(document).on("keydown", function(evt){
        ...
        console.log($target);   //최종적으로 active가 들어간 곳을 찾는 것이 아니라 키보드 이벤트가 발생한 시점에서 active라는 클래스가 들어간 곳의 인덱스번호를 추출

        if(key_num == 40 || key_num == 34){ //상방향키 & pageUp 키보드 눌렀을 때
            try{
                if($target == $(elm).length - 1){

                }else{
                    $("html, body").stop().animate({scrollTop : $(elm).eq($target + 1).offset().top}, 250);
                    $(elm).removeClass("active")
        ...
        }else if(key_num == 38 || key_num == 33){   //하방향키 & pageDown 키보드를 눌렀을때
            try{
                if($target == 0){
        ...
        }else if(key_num == 36){    //"Home" 버튼 클릭시 맨처음으로 이동
            try{
                $("html, body").stop().animate({scrollTop : $(elm).first().offset().top}, 250);
                $(elm).removeClass("active")
        ...
        }else if(key_num == 35){    //"End" 버튼 클릭시 맨마지막으로 이동
            try{
                $("html, body").stop().animate({scrollTop : $(elm).last().offset().top}, 250);
                $(elm).removeClass("active")
